export const TrigGraphMaterial = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
        ...
        float plotLine(float y, float linePos) {
            float lineWidth = 0.01; // Thickness of the line
        ...
        void main() {
            ...
            if (vUv.y < sectionHeight) {
                // Sin section (Red)
                color = mix(color, vec3(1.0, 0.0, 0.0), plotLine(vUv.y, sectionHeight / 2.0 + sin(x) * 0.07 * sin(time)));
            } else if (vUv.y < 2.0 * sectionHeight) {
                // Cos section (Green)
                color = mix(color, vec3(0.0, 1.0, 0.0), plotLine(vUv.y, sectionHeight * 1.5 + cos(x) * 0.07 * sin(time)));
            } else if (vUv.y < 3.0 * sectionHeight) {
                // Tan section (Blue)
                color = mix(color, vec3(0.0, 0.0, 1.0), plotLine(vUv.y, sectionHeight * 2.5 + tan(x) * 0.07 * sin(time)));
            } else if (vUv.y < 4.0 * sectionHeight) {
                // Csc section (Yellow)
                color = mix(color, vec3(1.0, 1.0, 0.0), plotLine(vUv.y, sectionHeight * 3.5 + 1.0 / sin(x) * 0.05 * sin(time)));
            } else if (vUv.y < 5.0 * sectionHeight) {
                // Sec section (Magenta)
                color = mix(color, vec3(1.0, 0.0, 1.0), plotLine(vUv.y, sectionHeight * 4.5 + 1.0 / cos(x) * 0.05 * sin(time)));
            } else {
                // Cot section (Cyan)
                color = mix(color, vec3(0.0, 1.0, 1.0), plotLine(vUv.y, sectionHeight * 5.5 + 1.0 / tan(x) * 0.05 * sin(time)));
            }

            // Add black separator lines between sections
            float lineWidth = 0.012;
            if (mod(vUv.y, sectionHeight) < lineWidth) {
